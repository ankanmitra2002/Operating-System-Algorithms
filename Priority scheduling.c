...
void heapify(Queue *arr,int n,int i)
{
	...
	int lchild = 2*i + 1;
	int rchild = 2*i + 2;
	if((lchild < n) && (arr[lchild].priority < arr[small].priority))
	    small = lchild;
	if((rchild < n) && (arr[rchild].priority < arr[small].priority))
	    small = rchild;
	if(small != i)
	{
...
int main()
{
    ...
    Queue prev;
    printf("Enter process number, burst time and its priority:\n");
    for(int i = 0 ; i < num ; i++)
    {
    	scanf("%d %d %d",&queue[i].p_id,&queue[i].burst_time,&queue[i].priority);
	}
	buildheap(queue,num);
